...
def _database_url() -> str:
    url = settings.VIBES_DATABASE_URL.strip()
    if url:
        return url
    if settings.is_production:
        raise RuntimeError("VIBES_DATABASE_URL is required for the Vibes service")
...
if database_url.startswith(("postgresql://", "postgresql+")):
    ...
    connect_args["application_name"] = "funkey-vibes"
    ...
    pool_options = {
        "pool_size": settings.VIBES_DB_POOL_SIZE,
        "max_overflow": settings.VIBES_DB_MAX_OVERFLOW,
        "pool_timeout": settings.VIBES_DB_POOL_TIMEOUT_SECONDS,
