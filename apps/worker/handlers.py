...
class VibesPostPublished(BaseModel):
    post_id: int = Field(gt=0)


class VibesMediaRequested(BaseModel):
    action: str = Field(pattern="^(link|delete)$")
    post_id: int = Field(gt=0)
...
def _processed(event: EventEnvelope, handler: str) -> bool:
    with SessionLocal() as db:
        return db.get(
            ...
            {"event_id": str(event.event_id), "handler": handler},
        ) is not None
...
def _vibes_fanout_snapshot(post_id: int) -> dict:
    ...
    try:
        response = requests.get(
            url,
            headers={
                "X-FunKey-Internal-Token": settings.VIBES_INTERNAL_TOKEN,
                "Accept": "application/json",
            },
            timeout=settings.VIBES_SERVICE_TIMEOUT_SECONDS,
        )
    except requests.RequestException as exc:
        raise RuntimeError("Vibes service unavailable") from exc
    if response.status_code < 200 or response.status_code >= 300:
        raise RuntimeError(f"Vibes fanout lookup failed: {response.status_code}")
    payload = response.json()
    if not isinstance(payload, dict):
        raise ValueError("Invalid Vibes fanout payload")
    return payload


def handle_vibes_post_published(event: EventEnvelope) -> str:
    payload = VibesPostPublished.model_validate(event.payload)
    handler_name = "vibes.post.published"
    if _processed(event, handler_name):
        return "duplicate"

    snapshot = _vibes_fanout_snapshot(payload.post_id)
    direct_ids = {int(value) for value in snapshot.get("direct_mention_user_ids", [])}
    mention_all_ids = {
        int(value) for value in snapshot.get("mention_all_user_ids", [])
    } - direct_ids
    author_user_id = int(snapshot["author_user_id"])
    author_public_user_id = int(snapshot["author_public_user_id"])
    author_name = str(snapshot.get("author_name") or "Vibe User")
    caption_preview = str(snapshot.get("caption_preview") or "")

    marker_key = {"event_id": str(event.event_id), "handler": handler_name}
    with SessionLocal() as db:
        try:
            db.add(WorkerProcessedEvent(**marker_key))
            db.flush()
            ...
            for user_id in sorted(direct_ids):
                db.add(UserNotification(
                    ...
                    title=f"{author_name} mentioned you"[:160],
                    body=f'Mentioned you in a Vibe: "{caption_preview}"',
                    target_type="vibe",
                    target_id=str(payload.post_id),
                    metadata_json=dict(common_metadata),
                    is_read=False,
                ))
            for user_id in sorted(mention_all_ids):
                db.add(UserNotification(
                    ...
                    title=f"{author_name} posted to followers"[:160],
    ...
    # The mention Inbox card is a secondary convenience projection. Existing
    # behavior already tolerated Inbox delivery failure; keep that property
    # while moving the work off the Vibes request path.
    inbox_text = (
        f"Mentioned you in a Vibe\n\n{caption_preview}\n\n"
        f"Vibe ID: {payload.post_id}"
    )
    for user_id in sorted(direct_ids):
        try:
            inbox_service_client.send_direct_message(
                ...
                message_type=(
                    "image"
                    if snapshot.get("media_type") == "photo"
                    and snapshot.get("media_url")
                    else "text"
                ),
...
def handle_vibes_media_requested(event: EventEnvelope) -> str:
    payload = VibesMediaRequested.model_validate(event.payload)
    handler_name = "vibes.media.requested"
    if _processed(event, handler_name):
        return "duplicate"

    # CDN-media helpers are idempotent for link/delete state. They currently
    # own their commits, so the processed marker is written after the side
    # effect; a crash can repeat the safe operation but cannot lose it.
    with SessionLocal() as db:
        asset = cdn_media_service.link_media_to_entity(
            ...
            linked_entity_id=str(payload.post_id),
        )
        if payload.action == "delete" and asset is not None:
            cdn_media_service.mark_media_deleted(
                db,
                asset=asset,
                actor_user_id=event.actor_user_id,
                reason=payload.reason or "vibe_deleted",
            )
