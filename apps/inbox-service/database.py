...
def _database_url() -> str:
    url = settings.INBOX_DATABASE_URL.strip()
    if url:
        return url
    if settings.is_production:
        raise RuntimeError("INBOX_DATABASE_URL is required for the Inbox service")
...
if database_url.startswith(("postgresql://", "postgresql+")):
    ...
    connect_args["application_name"] = "funkey-inbox"
    ...
    pool_options = {
        "pool_size": settings.INBOX_DB_POOL_SIZE,
        "max_overflow": settings.INBOX_DB_MAX_OVERFLOW,
        "pool_timeout": settings.INBOX_DB_POOL_TIMEOUT_SECONDS,
