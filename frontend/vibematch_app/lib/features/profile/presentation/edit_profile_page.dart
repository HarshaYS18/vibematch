...
class _EditProfilePageState extends State<EditProfilePage> {
  ...
  Future<void> _pickAndUploadAvatar() async {
    ...
    try {
      final upload = await _mediaUploadService.pickCropAndUploadAvatar(context);
      if (!mounted) return;
      setState(() => _avatarUrl = upload.url);
      _toast('Avatar cropped and uploaded. Tap Save to apply.');
  ...
  Future<void> _saveProfile() async {
    ...
    try {
      await _profileApi.updateMyProfile(
        ...
        coverPhotoUrls: widget.user.coverPhotoUrls,
        dateOfBirth: _dob,
  ...
  Future<void> _openDobPicker() async {
    ...
    await showModalBottomSheet<void>(context: context, backgroundColor: Colors.transparent, builder: (_) => _SheetShell(child: SizedBox(height: 320, child: Column(children: [Row(children: [const Expanded(child: Text('Select D.O.B', style: TextStyle(color: Color(0xFF251538), fontSize: 20, fontWeight: FontWeight.w900))), TextButton(onPressed: () { setState(() => _dob = selected); Navigator.pop(context); }, child: const Text('Done', style: TextStyle(fontWeight: FontWeight.w900)))]), Expanded(child: CupertinoDatePicker(mode: CupertinoDatePickerMode.date, initialDateTime: selected, minimumDate: DateTime(1950), maximumDate: DateTime.now(), onDateTimeChanged: (value) => selected = value))]))));
  }

  @override
  Widget build(BuildContext context) {
    final avatarName = _nameController.text.trim();
    return Scaffold(backgroundColor: const Color(0xFFFAF7F1), body: SafeArea(child: Column(children: [
      _Header(onBack: () => Navigator.pop(context), onSave: _saveProfile, saving: _saving),
      Expanded(child: ListView(physics: const BouncingScrollPhysics(), padding: const EdgeInsets.fromLTRB(16, 10, 16, 24), children: [
        ...
        _EditSection(title: 'Basic', children: [
          ...
          Row(children: [Expanded(child: _ReadOnlyInfoTile(label: 'Age', value: _age?.toString() ?? '', helper: _age == null ? 'Select D.O.B' : 'From D.O.B')), const SizedBox(width: 10), Expanded(child: _PickerTile(label: 'D.O.B', value: _dob == null ? 'Select D.O.B' : _formatDob(_dob!), onTap: _openDobPicker))]),
        ...
        const SizedBox(height: 12),
        _EditSection(title: 'Identity', children: [_DropdownTile(label: 'Profession', value: _profession.isEmpty ? null : _profession, values: profileProfessionOptions.map((item) => item.label).toList(), onChanged: (value) => setState(() => _profession = value)), _OptionWrap<MaritalStatus>(label: 'Marital status', values: MaritalStatus.values, selected: _maritalStatus, textFor: (item) => item.label, onSelected: (item) => setState(() => _maritalStatus = item))]),
        const SizedBox(height: 12),
        _EditSection(title: 'Interests', subtitle: 'Pick real interests for profile display and match percentage.', children: [for (final category in profileInterestCategories) _InterestCategoryBlock(category: category, selected: _interests, onToggle: (interest) { setState(() { if (_interests.contains(interest)) { _interests.remove(interest); } else { _interests.add(interest); } }); })]),
        const SizedBox(height: 12),
        _EditSection(title: 'Friend preference', subtitle: 'Used to calculate viewer-specific match percentage on public profiles.', children: [_OptionWrap<FriendGenderPreference>(label: 'Prefer to make friends with', values: FriendGenderPreference.values, selected: _friendGenderPreference, textFor: (item) => item.label, onSelected: (item) => setState(() => _friendGenderPreference = item)), _OptionWrap<FriendMaritalPreference>(label: 'Prefer based on marital status', values: FriendMaritalPreference.values, selected: _friendMaritalPreference, textFor: (item) => item.label, onSelected: (item) => setState(() => _friendMaritalPreference = item))]),
      ])),
    ])));
  }
