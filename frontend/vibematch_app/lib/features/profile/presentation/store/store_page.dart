...
class _VmStorePageState extends State<VmStorePage> {
  ...
  Future<void> _loadStore() async {
    ...
    final catalog = await VmStoreRepository.loadCatalog(
      hasLoveRelationship: userState.hasLoveRelationship,
    );
    if (!mounted) return;
    setState(() {
  ...
  Future<void> _buyItem(VmStoreItem item) async {
    ...
    try {
      ...
      final catalog = await VmStoreRepository.loadCatalog(
        hasLoveRelationship: next.hasLoveRelationship,
      );
      if (!mounted) return;
      setState(() {
        _userState = next;
  ...
  Future<void> _toggleLoveRelationshipForDebug() async {
    ...
    _toast(
      next.hasLoveRelationship
          ? 'Love card hidden because Lover bond exists.'
          : 'Love card available because no Lover bond exists.',
    );
  }
                    ...
                    if (_visibleItems.isEmpty)
                      ...
                      GridView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          final equipped = state.inventory.any(
                            (entry) =>
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      'Coins: ${_formatCoins(coinBalance)} - Mine: $ownedCount items',
          ...
          const SizedBox(height: 12),
          const Text(
            'Items use remote asset/config keys so frames, effects, bubbles, backgrounds and gradient names can be updated later without an app update.',
            style: TextStyle(
              color: Color(0xFF7B6A86),
              fontSize: 12,
              height: 1.35,
              ...
              child: Row(
                children: [
                  ...
                  Expanded(
                    child: Text(
                      hasLoveRelationship
                          ? 'Love Card availability: 0 because Lover bond already exists.'
                          : 'Love Card availability: 1 because no Lover bond exists.',
                      style: const TextStyle(
                        color: Color(0xFF251538),
                        fontSize: 11.5,
                        fontWeight: FontWeight.w900,
      ...
      child: Row(
        children: [
          Expanded(
            child: _TabButton(
              label: 'Store',
              active: selected == VmStoreTab.store,
              onTap: () => onSelected(VmStoreTab.store),
            ),
          ),
          Expanded(
            child: _TabButton(
              label: 'Mine',
              active: selected == VmStoreTab.mine,
              onTap: () => onSelected(VmStoreTab.mine),
            ),
          ),
        ],
      ),
  ...
  Widget build(BuildContext context) {
    return InkWell(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          color: active ? const Color(0xFF251538) : Colors.transparent,
          borderRadius: BorderRadius.circular(18),
        ),
        child: Text(
          label,
          style: TextStyle(
            color: active ? Colors.white : const Color(0xFF6B5C72),
            fontWeight: FontWeight.w900,
          ),
        ),
      ),
              ...
              child: Row(
                ...
                children: [
                  Icon(
                    section.icon,
                    size: 16,
                    color: active ? Colors.white : const Color(0xFF6D5DF6),
                  ),
...
class _RemoteConfigNotice extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final specialText = section == VmStoreSection.specialItems
        ? ' Gradient names are 100,000 coins each for 30 days.'
        : '';
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: const Color(0x3312C7B7)),
      ),
      child: Row(
        children: [
          const Icon(
          ...
          Expanded(
            child: Text(
              '${section.label} uses remote item keys. Backend/CDN can change active items, prices, previews and durations without app update.$specialText',
              style: const TextStyle(
                color: Color(0xFF064D46),
                fontSize: 11.5,
                height: 1.25,
                fontWeight: FontWeight.w800,
              ),
...
class _StoreItemCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final gradientStyleId = item.type == VmStoreItemType.gradientName
        ? item.previewAssetKey.split('/').last
        : null;
    final previewAsset = item.previewAssetKey.startsWith('assets/')
            ...
            child: Stack(
              ...
              children: [
                ...
                Positioned(
                  top: 8,
                  left: 8,
                  child: _TinyBadge(label: item.rarity.label),
                ),
                if (item.isDynamicRemoteItem)
                  const Positioned(
                    top: 8,
                    right: 8,
                    child: _TinyBadge(label: 'Remote'),
                  ),
                Positioned(
                  left: 10,
                  right: 10,
              ...
              children: [
                ...
                Wrap(
                  ...
                  children: [
                    if (item.durationDays != null)
                      _SmallMeta(label: '${item.durationDays}d'),
                    if (item.requiredVipLevel > 0)
                      _SmallMeta(label: 'VIP ${item.requiredVipLevel}'),
                    if (item.requiredSvipLevel > 0)
                      _SmallMeta(label: 'SVIP ${item.requiredSvipLevel}'),
                ...
                _StoreActionButton(
                  label: equipped
                      ...
                      : item.isFree
                      ? 'Free'
                      : '${_formatCoins(item.priceCoins)} coins',
                  enabled: !busy,
                  filled: !equipped,
                  onTap: equipped
                      ? onUnequip
  ...
  Widget build(BuildContext context) {
    return InkWell(
      ...
      child: Container(
        ...
        alignment: Alignment.center,
        decoration: BoxDecoration(
          color: filled ? const Color(0xFF251538) : const Color(0xFFE9FBF8),
          borderRadius: BorderRadius.circular(16),
          border: Border.all(
            color: filled ? const Color(0xFF251538) : const Color(0xFF12C7B7),
          ),
        ),
        child: Text(
          label,
          ...
          overflow: TextOverflow.ellipsis,
          style: TextStyle(
            color: filled ? Colors.white : const Color(0xFF064D46),
            fontSize: 12,
            fontWeight: FontWeight.w900,
          ),
        ),
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          Text(
            isMine ? 'No owned ${section.label}' : 'No active ${section.label}',
            textAlign: TextAlign.center,
            style: const TextStyle(
              color: Color(0xFF251538),
              fontSize: 16,
          ...
          Text(
            isMine
                ? 'Items you buy in this section will appear here.'
                : 'Backend can activate new items in this section without app update.',
            textAlign: TextAlign.center,
            style: const TextStyle(
              color: Color(0xFF7B6A86),
              fontSize: 12,
