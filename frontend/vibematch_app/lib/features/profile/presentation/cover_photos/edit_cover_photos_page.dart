...
class _EditCoverPhotosPageState extends State<EditCoverPhotosPage> {
  ...
  Future<void> _addCoverPhoto() async {
    ...
    try {
      final upload = await _mediaUploadService.pickCropAndUploadProfileCover(
        context,
      );
      if (!mounted) return;
      setState(() => _coverPhotoUrls.add(upload.url));
      final saved = await _syncCoverPhotos(
        successMessage: 'Cover photo updated across the app.',
        allowWhileUploading: true,
      );
      if (!saved && mounted) {
        setState(() => _coverPhotoUrls.remove(upload.url));
      }
  ...
  Future<void> _deleteCoverPhoto(int index) async {
    if (_isSaving ||
        _isUploading ||
        index < 0 ||
        index >= _coverPhotoUrls.length)
      return;
    final removedUrl = _coverPhotoUrls[index];
    setState(() => _coverPhotoUrls.removeAt(index));
    final saved = await _syncCoverPhotos(
      successMessage: 'Cover photo removed across the app.',
    );
    if (!saved && mounted) {
      setState(() => _coverPhotoUrls.insert(index, removedUrl));
    }
  }

  Future<void> _saveCoverPhotos() async {
    await _syncCoverPhotos(closeAfterSave: true);
  }

  Future<bool> _syncCoverPhotos({
    bool closeAfterSave = false,
    String successMessage = 'Cover photos saved.',
    bool allowWhileUploading = false,
  }) async {
    if (_isSaving || (_isUploading && !allowWhileUploading)) return false;
...
BoxDecoration _coverCardDecoration() => BoxDecoration(
  color: Colors.white,
  borderRadius: BorderRadius.circular(26),
