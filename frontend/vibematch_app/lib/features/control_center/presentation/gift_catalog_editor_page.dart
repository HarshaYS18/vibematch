...
class _GiftCatalogEditorPageState extends State<GiftCatalogEditorPage> {
  ...
  List<Map<String, dynamic>> get _items =>
      ((_catalog?['items'] as List<dynamic>? ?? _catalog?['all'] as List<dynamic>? ?? const <dynamic>[])
              .whereType<Map<String, dynamic>>()
              .toList())
          ..sort((a, b) {
            final left = _int(a['sort_order']);
  ...
  Future<void> _runMutation(Future<void> Function(String reason) action, String title, String hint) async {
    ...
    final shouldSave = await showDialog<bool>(
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, setLocal) => AlertDialog(
          title: Text(category == null ? 'Add category' : 'Edit category', style: const TextStyle(fontWeight: FontWeight.w900)),
          content: SizedBox(
            width: 440,
  ...
  Future<void> _openGiftEditor([Map<String, dynamic>? gift]) async {
    ...
    final shouldSave = await showDialog<bool>(
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, setLocal) => AlertDialog(
          title: Text(gift == null ? 'Add gift' : 'Edit gift', style: const TextStyle(fontWeight: FontWeight.w900)),
          content: SizedBox(
            width: 560,
            child: SingleChildScrollView(
              child: Column(mainAxisSize: MainAxisSize.min, children: [
...
class _HeaderCard extends StatelessWidget {
  const _HeaderCard({required this.source, required this.cdnBase, required this.itemCount, required this.categoryCount, required this.onSeed, required this.onAddCategory});
  final String source;
...
class _SectionTitle extends StatelessWidget {
  ...
  final String text;
  @override
  Widget build(BuildContext context) => Text(text, style: const TextStyle(color: Color(0xFF251538), fontSize: 17, fontWeight: FontWeight.w900));
}
