...
class WalletApiService {
  ...
  Future<VmWallet> getWallet() async {
    final response = await http.get(Uri.parse(VmApiConfig.endpoint('/wallet/me')), headers: _headers());
  ...
  Future<List<VmWalletLedgerEntry>> getLedger({int limit = 50}) async {
    final response = await http.get(Uri.parse(VmApiConfig.endpoint('/wallet/ledger?limit=$limit')), headers: _headers());
    _throwIfBad(response, 'Failed to load wallet ledger');
    final decoded = jsonDecode(response.body) as List<dynamic>;
    return decoded.whereType<Map<String, dynamic>>().map(VmWalletLedgerEntry.fromJson).toList();
  }
...
class VmWalletLedgerEntry {
  ...
  factory VmWalletLedgerEntry.fromJson(Map<String, dynamic> json) => VmWalletLedgerEntry(
        id: _int(json['id']),
