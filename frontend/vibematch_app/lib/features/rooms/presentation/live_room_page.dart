...
class _LiveRoomPageState extends State<LiveRoomPage> {
  ...
  Timer? _roomPresenceHeartbeatTimer;
  bool _roomPresenceLeft = false;
  ...
  void dispose() {
    ...
    _roomPresenceHeartbeatTimer?.cancel();
    unawaited(_leaveRoomPresence());
  ...
  void _startRoomPresence() {
    _roomPresenceLeft = false;
    _roomPresenceHeartbeatTimer?.cancel();

    unawaited(_enterRoomPresence());

    _roomPresenceHeartbeatTimer = Timer.periodic(
      const Duration(seconds: 25),
      (_) => unawaited(_sendRoomPresenceHeartbeat()),
    );
  }
  ...
  Future<void> _sendRoomPresenceHeartbeat() async {
    if (_roomPresenceLeft) return;
