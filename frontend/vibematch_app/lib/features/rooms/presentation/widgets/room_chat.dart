  ...
  List<TextSpan> _messageSpans(
    ...
    for (final match in mentionRegex.allMatches(message.message)) {
      if (match.start > cursor)
        spans.add(
          _normalSpan(message.message.substring(cursor, match.start), message),
        );
    ...
    if (cursor < message.message.length)
      spans.add(_normalSpan(message.message.substring(cursor), message));
    return spans;
  }
...
class _ChatAvatar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final hasAvatar = safeUrl != null && safeUrl.isNotEmpty;
    ...
    return _ChatGlassBox(
      child: Row(
        ...
        children: [
          ...
          Icon(
            isRemove ? Icons.person_remove_alt_1_rounded : Icons.login_rounded,
            color: color,
            size: 15,
          ),
          const SizedBox(width: 6),
