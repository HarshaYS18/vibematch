...
class _LiveRoomPrivacySheetState extends State<LiveRoomPrivacySheet> {
  ...
  String get _roomId => (widget.roomId ?? ActiveRoomContext.roomPublicId ?? '').trim();
  ...
  Future<void> _saveAccessSettings({String? language, RoomPrivacyMode? mode, bool? allowScreenshots}) async {
    if (_saving) return;
    final roomId = _roomId;
    if (roomId.isEmpty) {
  ...
  String _successMessage({String? language, RoomPrivacyMode? mode, bool? allowScreenshots, required RoomPrivacyMode confirmedMode}) {
    if (language != null) return 'Room language updated to $language.';
    if (allowScreenshots != null) return allowScreenshots ? 'Screenshots are now allowed.' : 'Screenshots are now disabled.';
  ...
  void _openLanguageSheet() {
    showModalBottomSheet<void>(
      ...
      builder: (context) => Container(
        ...
        padding: EdgeInsets.fromLTRB(14, 12, 14, MediaQuery.paddingOf(context).bottom + 14),
        ...
        child: Column(
          ...
          children: [
            ...
            const Text('Room Language', style: TextStyle(color: RoomColors.plum, fontSize: 17, fontWeight: FontWeight.w900)),
              ...
              child: ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  return ListTile(
                    ...
                    leading: Icon(selected ? Icons.check_circle_rounded : Icons.language_rounded, color: selected ? RoomColors.aqua : RoomColors.plum, size: 20),
                    title: Text(language, style: TextStyle(color: RoomColors.plum, fontWeight: selected ? FontWeight.w900 : FontWeight.w700)),
                    onTap: () {
        ...
        child: Container(
          ...
          child: Column(
            ...
            children: [
              ...
              Row(
                children: [
                  const Expanded(child: Text('Room Privacy', style: TextStyle(color: RoomColors.plum, fontSize: 17, fontWeight: FontWeight.w900))),
                  if (_saving || _loadingSettings) const SizedBox(width: 18, height: 18, child: CircularProgressIndicator(strokeWidth: 2)),
                ],
              ),
              const SizedBox(height: 4),
              const Text('Language, screenshots and mode changes are saved to backend for this lifetime room.', style: TextStyle(color: Color(0xFF82758E), fontSize: 11.5, fontWeight: FontWeight.w800)),
              const SizedBox(height: 8),
              Expanded(
                child: ListView(
                  ...
                  children: [
                    _SettingsTile(
                      ...
                      trailing: const Icon(Icons.chevron_right_rounded, color: Color(0xFF82758E), size: 20),
                      onTap: _saving ? null : _openLanguageSheet,
                    ),
                    _ScreenshotTile(
                      value: _allowScreenshots,
                      enabled: !_saving,
                      onChanged: (value) => unawaited(_saveAccessSettings(allowScreenshots: value)),
                    ),
                    const SizedBox(height: 8),
                    ...RoomPrivacyMode.values.map((mode) {
                      return _PrivacyTile(
                        ...
                        onTap: () {
                          setState(() => _mode = mode);
                          if (mode != RoomPrivacyMode.locked) unawaited(_saveAccessSettings(mode: mode));
                        },
                      );
                    }),
                    if (_mode == RoomPrivacyMode.locked) ...[
                      ...
                      TextField(
                        ...
                        onSubmitted: (_) => unawaited(_saveAccessSettings(mode: RoomPrivacyMode.locked)),
                      ),
                      const SizedBox(height: 8),
                      SizedBox(
                        width: double.infinity,
                        child: ElevatedButton.icon(
                          onPressed: _saving ? null : () => unawaited(_saveAccessSettings(mode: RoomPrivacyMode.locked)),
                          style: ElevatedButton.styleFrom(backgroundColor: RoomColors.plum, foregroundColor: Colors.white, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(14)), elevation: 0),
  ...
  Widget build(BuildContext context) {
    return InkWell(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(color: const Color(0xFFFCFAF6), borderRadius: BorderRadius.circular(14), border: Border.all(color: const Color(0xFFE8DDCF))),
        child: Row(
          children: [
...
class _ScreenshotTile extends StatelessWidget {
  const _ScreenshotTile({required this.value, required this.enabled, required this.onChanged});
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Row(
        children: [
          Icon(value ? Icons.screenshot_monitor_rounded : Icons.no_photography_rounded, color: value ? RoomColors.aqua : RoomColors.coral, size: 18),
          const SizedBox(width: 9),
          Expanded(
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              const Text('Screenshots', style: TextStyle(color: RoomColors.plum, fontSize: 12.5, fontWeight: FontWeight.w900)),
              const SizedBox(height: 2),
              Text(value ? 'Allowed in this room' : 'Disabled in this room', style: const TextStyle(color: Color(0xFF82758E), fontSize: 10.8, fontWeight: FontWeight.w700)),
            ]),
          ),
          Switch(value: value, onChanged: enabled ? onChanged : null, activeThumbColor: RoomColors.aqua),
        ],
      ),
    );
  }
...
class _PrivacyTile extends StatelessWidget {
  const _PrivacyTile({required this.mode, required this.selected, required this.description, required this.saving, required this.onTap});
    ...
    return InkWell(
      ...
      child: Container(
        ...
        child: Row(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Padding(padding: const EdgeInsets.only(top: 1), child: Icon(mode.icon, color: selected ? RoomColors.aqua : RoomColors.plum, size: 18)),
            const SizedBox(width: 9),
            Expanded(
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text(mode.label, style: TextStyle(color: selected ? RoomColors.aqua : RoomColors.plum, fontSize: 12.5, fontWeight: FontWeight.w900)),
                const SizedBox(height: 2),
                Text(description, style: const TextStyle(color: Color(0xFF82758E), fontSize: 10.3, height: 1.18, fontWeight: FontWeight.w700)),
              ]),
            ),
            if (selected) const Padding(padding: EdgeInsets.only(top: 1), child: Icon(Icons.check_circle_rounded, color: RoomColors.aqua, size: 18)),
          ],
        ),
      ),
    );
