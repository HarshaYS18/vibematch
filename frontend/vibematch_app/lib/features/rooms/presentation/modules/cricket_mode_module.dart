...
enum CricketWicketType { bowled, caught, lbw, runOut, stumped, hitWicket, retired }
...
class CricketBallEvent {
  ...
  String get chip {
    ...
    if (extraType == CricketExtraType.wide) return extrasRuns > 1 ? 'Wd+$runsBat' : 'Wd';
...
class CricketMatchState {
  ...
  List<CricketBallEvent> get inningsEvents => events.where((e) => e.innings == innings).toList(growable: false);

  CricketPlayer playerById(String id) {
    return [...teamA.players, ...teamB.players].firstWhere(
      (player) => player.id == id,
      orElse: () => CricketPlayer(id: id, name: 'Player', teamId: battingTeamId),
    );
  }
...
class CricketScoringEngine {
  ...
  static CricketMatchState demo({required String roomId, required String roomName}) {
    final teamAPlayers = List<CricketPlayer>.generate(
      11,
      (index) => CricketPlayer(
        id: 'a_${index + 1}',
        name: ['Arjun', 'Dev', 'Kiran', 'Manoj', 'Sai', 'Rohit', 'Varun', 'Nikhil', 'Yash', 'Ravi', 'Ajay'][index],
        teamId: 'team_a',
    ...
    final teamBPlayers = List<CricketPlayer>.generate(
      ...
      (index) => CricketPlayer(
        id: 'b_${index + 1}',
        name: ['Ravi', 'Bala', 'Surya', 'Mahesh', 'Vikram', 'Charan', 'Teja', 'Pavan', 'Abhi', 'Karthik', 'Venu'][index],
        teamId: 'team_b',
        battingOrder: index + 1,
      ),
    );
    ...
    return CricketMatchState(
      roomId: roomId,
      roomName: roomName,
      teamA: CricketTeam(id: 'team_a', name: 'Vibe Strikers', shortName: 'VBS', players: teamAPlayers),
      teamB: CricketTeam(id: 'team_b', name: 'Royal Hitters', shortName: 'RHT', players: teamBPlayers),
      oversLimit: 5,
      ballsPerOver: 6,
      totalWickets: 10,
  ...
  static CricketScoreSnapshot snapshot(CricketMatchState state) {
    ...
    final currentRunRate = legalBalls == 0 ? 0.0 : runs / (legalBalls / state.ballsPerOver);
    final ballsLeft = math.max(0, (state.oversLimit * state.ballsPerOver) - legalBalls);
    final required = state.targetRuns == null || ballsLeft == 0
        ? null
        : math.max(0, state.targetRuns! - runs) / (ballsLeft / state.ballsPerOver);
  ...
  static CricketMatchState endInnings(CricketMatchState state) {
    if (state.innings == 2) return state.copyWith(status: CricketMatchStatus.completed);
  ...
  static CricketMatchState _appendBall(
    ...
    final legalBalls = state.inningsEvents.where((event) => event.isLegalBall).length;
    final nextLegalBall = legalBalls + (isLegalBall ? 1 : 0);
    final event = CricketBallEvent(
      sequence: state.events.length + 1,
      innings: state.innings,
      over: legalBalls ~/ state.ballsPerOver,
      ball: isLegalBall ? ((nextLegalBall - 1) % state.ballsPerOver) + 1 : (legalBalls % state.ballsPerOver) + 1,
      ...
      commentary: _commentary(state, runsBat, extrasRuns, extraType, wicketType),
    );
    ...
    if (wicketType != null && nextBatterIndex < state.battingTeam.players.length) {
      striker = state.battingTeam.players[nextBatterIndex].id;
      nextBatterIndex += 1;
    } else if ((runsBat + (extraType == CricketExtraType.bye || extraType == CricketExtraType.legBye ? extrasRuns : 0)).isOdd) {
      final temp = striker;
      striker = nonStriker;
      nonStriker = temp;
    }
  ...
  static CricketMatchState _replay(CricketMatchState seed, List<CricketBallEvent> events) {
  ...
  static String _commentary(CricketMatchState state, int runsBat, int extrasRuns, CricketExtraType? extraType, CricketWicketType? wicketType) {
    final striker = state.playerById(state.strikerId).name;
    final bowler = state.playerById(state.bowlerId).name;
    if (wicketType != null) return '$bowler to $striker, OUT! ${_wicketLabel(wicketType)}.';
    if (extraType != null) return '$bowler to $striker, ${_extraLabel(extraType)} $extrasRuns.';
...
class _CricketModeSheetState extends State<CricketModeSheet> {
  ...
  RoomBackgroundTheme _selectedCricketBackground = cricketFloodlightArenaBackgroundTheme;

  @override
  void initState() {
    super.initState();
    _controller = CricketModeController(roomId: widget.roomId, roomName: widget.roomName)..addListener(_onChanged);
  }
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          _Header(status: state.status, canManage: widget.canManage, onEnd: _endMode),
          const SizedBox(height: 12),
          _ScorePanel(state: state, snapshot: snapshot),
          const SizedBox(height: 10),
          if (state.status == CricketMatchStatus.setup) _SetupPanel(selected: _selectedCricketBackground, onSelected: (theme) => setState(() => _selectedCricketBackground = theme), onStart: _startMode),
          if (state.status == CricketMatchStatus.live) _ScorerKeypad(controller: _controller, canManage: widget.canManage),
          if (state.status == CricketMatchStatus.inningsBreak) _InningsBreakPanel(state: state, onStartSecondInnings: _controller.startSecondInnings, onComplete: _controller.complete),
          if (state.status == CricketMatchStatus.completed) _CompletedPanel(state: state),
          const SizedBox(height: 10),
          Expanded(child: _CommentaryList(events: state.events.reversed.toList(growable: false))),
        ],
      ),
    );
  }
...
class _Header extends StatelessWidget {
  const _Header({required this.status, required this.canManage, required this.onEnd});
      ...
      children: [
        const Expanded(
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text('Cricket Mode', style: TextStyle(color: RoomColors.plum, fontSize: 20, fontWeight: FontWeight.w900, letterSpacing: -0.4)),
              SizedBox(height: 3),
              Text('Ball-by-ball scorer • live room mode • backend-ready events', style: TextStyle(color: Color(0xFF7C7186), fontSize: 11.5, fontWeight: FontWeight.w800)),
            ],
          ),
        ),
        if (status == CricketMatchStatus.live || status == CricketMatchStatus.inningsBreak)
          TextButton.icon(
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: const LinearGradient(colors: [Color(0xFF062611), Color(0xFF0E5930)]),
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text('${state.battingTeam.shortName} ${snapshot.runs}/${snapshot.wickets} (${snapshot.oversText})', style: const TextStyle(color: Colors.white, fontSize: 27, fontWeight: FontWeight.w900, letterSpacing: -0.8)),
          const SizedBox(height: 5),
          Text(state.targetRuns == null ? '${state.battingTeam.name} batting' : 'Target ${state.targetRuns} • Need ${math.max(0, state.targetRuns! - snapshot.runs)}', style: TextStyle(color: Colors.white.withValues(alpha: 0.82), fontSize: 12, fontWeight: FontWeight.w800)),
          const SizedBox(height: 10),
          Wrap(
            spacing: 7,
            runSpacing: 7,
            children: [
              ...
              _InfoChip(label: 'CRR ${snapshot.currentRunRate.toStringAsFixed(2)}'),
              if (snapshot.requiredRunRate != null) _InfoChip(label: 'RRR ${snapshot.requiredRunRate!.toStringAsFixed(2)}'),
            ],
          ),
          if (snapshot.recentBalls.isNotEmpty) ...[
            const SizedBox(height: 10),
            Wrap(spacing: 6, children: snapshot.recentBalls.map((ball) => _BallChip(ball)).toList()),
          ],
        ],
      ),
    );
...
class _InfoChip extends StatelessWidget {
  ...
  final String label;
  @override
  Widget build(BuildContext context) => Container(padding: const EdgeInsets.symmetric(horizontal: 9, vertical: 5), decoration: BoxDecoration(color: Colors.white.withValues(alpha: 0.14), borderRadius: BorderRadius.circular(999), border: Border.all(color: Colors.white.withValues(alpha: 0.16))), child: Text(label, style: const TextStyle(color: Colors.white, fontSize: 11, fontWeight: FontWeight.w900)));
}
...
class _BallChip extends StatelessWidget {
  ...
  final String label;
  @override
  Widget build(BuildContext context) => Container(width: 30, height: 30, alignment: Alignment.center, decoration: const BoxDecoration(color: Colors.white, shape: BoxShape.circle), child: Text(label, style: const TextStyle(color: Color(0xFF0E5930), fontSize: 10.5, fontWeight: FontWeight.w900)));
}

class _SetupPanel extends StatelessWidget {
  const _SetupPanel({required this.selected, required this.onSelected, required this.onStart});
  ...
  Widget build(BuildContext context) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Text('Cricket backgrounds', style: TextStyle(color: RoomColors.plum, fontSize: 13, fontWeight: FontWeight.w900)),
        ...
        const SizedBox(height: 10),
        SizedBox(width: double.infinity, height: 46, child: FilledButton.icon(onPressed: onStart, icon: const Icon(Icons.sports_cricket_rounded), label: const Text('Start Cricket Mode'))),
      ],
    );
  }
}
        ...
        child: Column(
          children: [
            GridView.count(
              ...
              children: [0, 1, 2, 3, 4, 5, 6].map((run) => _ScoreButton(label: '$run', onTap: () => controller.addRuns(run))).toList()
                ..addAll([
                  _ScoreButton(label: 'Wd', onTap: () => controller.addExtra(CricketExtraType.wide, 1)),
                  _ScoreButton(label: 'Nb', onTap: () => controller.addExtra(CricketExtraType.noBall, 1)),
                  _ScoreButton(label: 'Bye', onTap: () => controller.addExtra(CricketExtraType.bye, 1)),
                  _ScoreButton(label: 'LB', onTap: () => controller.addExtra(CricketExtraType.legBye, 1)),
                  _ScoreButton(label: 'Wicket', danger: true, onTap: () => controller.addWicket(CricketWicketType.bowled)),
                ]),
            ),
            const SizedBox(height: 8),
            Row(
              children: [
                Expanded(child: OutlinedButton.icon(onPressed: controller.undo, icon: const Icon(Icons.undo_rounded), label: const Text('Undo'))),
                const SizedBox(width: 8),
                Expanded(child: OutlinedButton.icon(onPressed: controller.endInnings, icon: const Icon(Icons.flag_rounded), label: const Text('End innings'))),
              ],
            ),
          ],
        ),
...
class _ScoreButton extends StatelessWidget {
  const _ScoreButton({required this.label, required this.onTap, this.danger = false});
  final String label;
  ...
  Widget build(BuildContext context) => Material(
        ...
        child: InkWell(
          borderRadius: BorderRadius.circular(16),
          onTap: onTap,
          child: Center(child: Text(label, style: TextStyle(color: danger ? Colors.white : RoomColors.plum, fontSize: 15, fontWeight: FontWeight.w900))),
        ),
      );
}

class _InningsBreakPanel extends StatelessWidget {
  const _InningsBreakPanel({required this.state, required this.onStartSecondInnings, required this.onComplete});
  final CricketMatchState state;
  ...
  Widget build(BuildContext context) => Row(
        children: [
          Expanded(child: FilledButton(onPressed: state.innings == 1 ? onStartSecondInnings : onComplete, child: Text(state.innings == 1 ? 'Start 2nd Innings' : 'Complete Match'))),
        ],
      );
}
...
class _CompletedPanel extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
        width: double.infinity,
        padding: const EdgeInsets.all(14),
        decoration: BoxDecoration(color: RoomColors.gold.withValues(alpha: 0.20), borderRadius: BorderRadius.circular(18)),
        child: const Text('Match completed. Scorecard, player stats, and backend sync hooks are ready for connection.', style: TextStyle(color: RoomColors.plum, fontWeight: FontWeight.w900)),
      );
}
...
class _CommentaryList extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    if (events.isEmpty) {
      return const Center(child: Text('No balls yet. Start scoring to generate ball-by-ball commentary.', style: TextStyle(color: Color(0xFF81758C), fontWeight: FontWeight.w800)));
    }
    return ListView.separated(
      ...
      itemBuilder: (context, index) {
        ...
        return ListTile(
          ...
          title: Text(event.commentary, style: const TextStyle(color: RoomColors.plum, fontSize: 12.5, fontWeight: FontWeight.w800)),
          subtitle: Text('${event.over}.${event.ball} • Innings ${event.innings}', style: const TextStyle(fontSize: 10.5, fontWeight: FontWeight.w700)),
        );
      },
    );
  }
