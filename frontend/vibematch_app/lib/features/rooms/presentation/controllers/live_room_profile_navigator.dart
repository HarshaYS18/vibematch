...
class LiveRoomProfileNavigator {
  ...
  static void openFollowersPage({required BuildContext context, required SeatUser user, required List<SeatUser> users}) {
    _pushRoomActionPageFromSheet(context, FollowersFollowedPage(user: user, users: users, initialTabIndex: 0));
  }

  static void openFollowedPage({required BuildContext context, required SeatUser user, required List<SeatUser> users}) {
    _pushRoomActionPageFromSheet(context, FollowersFollowedPage(user: user, users: users, initialTabIndex: 1));
  }

  static void openVipCentrePage({required BuildContext context, required SeatUser user}) {
    _openViewerVipProgram(context: context, initialTabIndex: 0);
  }

  static void openSvipCentrePage({required BuildContext context, required SeatUser user}) {
    _openViewerVipProgram(context: context, initialTabIndex: 1);
  }

  static void _openViewerVipProgram({required BuildContext context, required int initialTabIndex}) {
  ...
  static void openSendingExperiencePage({required BuildContext context, required SeatUser user}) {
    _pushRoomActionPageFromSheet(context, ExperienceLevelPage(user: user, type: ExperienceLevelType.sent));
  }

  static void openReceivingExperiencePage({required BuildContext context, required SeatUser user}) {
    _pushRoomActionPageFromSheet(context, ExperienceLevelPage(user: user, type: ExperienceLevelType.received));
  }

  static void openSentRankingsPage({required BuildContext context, required List<SeatUser> users}) {
    _pushRoomActionPageFromSheet(context, RoomRankingsPage(title: 'Sent Rankings', users: users, sentRanking: true));
  }

  static void openReceivedRankingsPage({required BuildContext context, required List<SeatUser> users}) {
    _pushRoomActionPageFromSheet(context, RoomRankingsPage(title: 'Received Rankings', users: users, sentRanking: false));
  }

  static void openFamilyPage({required BuildContext context, required SeatUser user}) {
    ...
    _pushRoomActionPageFromSheet(
      ...
      RoomActionPage(
        ...
        cards: [
          ...
          const RoomActionCard(title: 'Family events', value: 'Family-vs-family activities and rewards connect here', icon: Icons.emoji_events_rounded, color: RoomColors.gold),
        ],
      ),
    );
  }

  static void openLoveAndBondCentre({required BuildContext context, required SeatUser user}) {
    _pushRoomActionPageFromSheet(
      context,
      RoomActionPage(
        ...
        cards: [
          ...
          const RoomActionCard(title: 'Cards', value: 'Relationship cards and bond actions connect here', icon: Icons.style_rounded, color: RoomColors.violet),
        ],
      ),
    );
  }

  static void openMedalsPage({required BuildContext context, required SeatUser user}) {
    _pushRoomActionPageFromSheet(
      context,
      RoomActionPage(
        title: 'Medals',
        ...
        cards: [
          ...
          const RoomActionCard(title: 'Achievement centre', value: 'Medal progress and rules connect here', icon: Icons.auto_graph_rounded, color: RoomColors.aqua),
        ],
      ),
    );
  }

  static void openModulePage({required BuildContext context, required String title, required String subtitle, required IconData icon}) {
    _pushRoomActionPageFromSheet(context, RoomActionPage(title: title, subtitle: subtitle, icon: icon));
  }
