...
class GiftCatalogApiService {
  ...
  Future<GiftCatalogSnapshot> fetchActiveCatalog() async {
    ...
    return GiftCatalogSnapshot.fromJson(json);
  }
}
...
class GiftCatalogSnapshot {
  ...
  factory GiftCatalogSnapshot.fromJson(Map<String, dynamic> json) {
    final rawCategories = (json['categories'] as List<dynamic>? ?? const <dynamic>[])
        .whereType<Map<String, dynamic>>()
        .map(GiftCatalogCategory.fromJson)
        .where((category) => category.isEnabled)
        .toList(growable: false);

    final rawGifts = (json['all'] as List<dynamic>? ?? const <dynamic>[])
        .whereType<Map<String, dynamic>>()
        .map(_giftFromJson)
        .toList(growable: false);
...
GiftItem _giftFromJson(Map<String, dynamic> json) {
  ...
  return GiftItem(
    ...
    colors: _colorsFor(categoryKey, isPremium: isPremium, isLucky: isLucky),
    giftType: giftType,
...
GiftCategory _categoryFromKey(String key) {
  switch (key) {
    case 'classic':
      return GiftCategory.classic;
    case 'lucky':
      return GiftCategory.lucky;
    case 'relationship':
      return GiftCategory.relationship;
    case 'event':
      return GiftCategory.event;
    case 'premium':
      return GiftCategory.premium;
    case 'svip':
      return GiftCategory.svip;
    case 'vip':
      return GiftCategory.vip;
    case 'baggage':
      return GiftCategory.baggage;
    default:
      return GiftCategory.classic;
  }
}
...
IconData _iconFor(String? iconKey, String categoryKey) {
  switch (iconKey) {
    ...
    case 'local_celebration':
      return Icons.celebration_rounded;
    case 'local_favorite':
    case 'local_favorite_border':
      return Icons.favorite_rounded;
    case 'local_auto_fix_high':
      return Icons.auto_fix_high_rounded;
    case 'local_auto_awesome':
      return Icons.auto_awesome_rounded;
    case 'local_workspace_premium':
      return Icons.workspace_premium_rounded;
    case 'local_flutter_dash':
...
List<Color> _colorsFor(String categoryKey, {required bool isPremium, required bool isLucky}) {
  ...
  switch (categoryKey) {
    case 'event':
      return const <Color>[Color(0xFFFF7A45), Color(0xFF8C5CF6)];
    case 'svip':
      return const <Color>[Color(0xFF8C5CF6), Color(0xFF111827)];
    case 'vip':
      return const <Color>[Color(0xFF12C7B7), Color(0xFFFFD166)];
    case 'baggage':
      return const <Color>[Color(0xFF18C7B7), Color(0xFF6C63FF)];
  }
