...
class _InboxSettingsPageState extends State<InboxSettingsPage> {
  final InboxPreferencesApiService _preferencesApi =
      const InboxPreferencesApiService();
  ...
  void _setStrangersCanMentionInVibes(bool value) {
    ...
    _savePreferences(
      ...
      feedback: value
          ? 'Strangers can mention you in Vibes'
          : 'Stranger Vibes mentions disabled',
    );
  }
          ...
          children: [
            ...
            _SettingsCard(
              child: Column(
                children: [
                  _ActionRow(
                    icon: lockStatus.isEnabled
                        ? Icons.lock_rounded
                        : Icons.lock_open_rounded,
                    title: lockStatus.isEnabled
                        ? 'Change Inbox lock'
                        : 'Set up Inbox lock',
                    subtitle: lockStatus.isEnabled
                        ? 'Use your current lock to set a new one. Support recovery codes work as backup.'
                        : 'Create your first Inbox lock with recovery mobile OTP.',
                    onTap: lockStatus.isEnabled
                        ? _openChangeLock
                        : _openLockSetup,
                  ),
                  ...
                  SwitchListTile(
                    ...
                    onChanged: _preferencesBusy
                        ? null
                        : (value) => _savePreferences(
                            prefs.copyWith(deviceUnlockEnabled: value),
                            feedback: value
                                ? 'Device unlock shortcut enabled'
                                : 'Device unlock shortcut disabled',
                          ),
                    activeThumbColor: const Color(0xFF12C7B7),
                    title: const Text(
                      'Device unlock shortcut',
                      style: TextStyle(
                        color: Color(0xFF251538),
                        fontWeight: FontWeight.w900,
                      ),
                    ...
                    subtitle: const Text(
                      'Uses phone fingerprint/Face ID locally after backend lock is configured. Fingerprint data is never sent to backend.',
                      style: TextStyle(
              ...
              child: Column(
                children: [
                  ...
                  SwitchListTile(
                    ...
                    onChanged: _preferencesBusy
                        ? null
                        : _setStrangersCanMentionInVibes,
                    activeThumbColor: const Color(0xFF12C7B7),
                    title: const Text(
                  ...
                  SwitchListTile(
                    ...
                    subtitle: const Text(
                      'Control whether people can see blue read ticks in direct chats.',
                      style: TextStyle(
                        color: Color(0xFF7B6A86),
                        fontSize: 11.5,
                        fontWeight: FontWeight.w700,
                  ...
                  _VisibilityRow(
                    ...
                    onChanged: (value) => _savePreferences(
                      prefs.copyWith(onlineVisibility: value),
                      feedback: 'Online status privacy updated',
                    ),
                  ),
                  const Divider(color: Color(0xFFECE2D8)),
                  _VisibilityRow(
                    ...
                    onChanged: (value) => _savePreferences(
                      prefs.copyWith(lastSeenVisibility: value),
                      feedback: 'Last seen privacy updated',
                    ),
                  ),
                  const Divider(color: Color(0xFFECE2D8)),
                  _VisibilityRow(
                    ...
                    onChanged: (value) => _savePreferences(
                      prefs.copyWith(typingActivityVisibility: value),
                      feedback: 'Typing privacy updated',
                    ),
                  ),
                  const Divider(color: Color(0xFFECE2D8)),
                  _VisibilityRow(
                    ...
                    onChanged: (value) => _savePreferences(
                      prefs.copyWith(storyVisibility: value),
                      feedback: 'Story privacy updated',
                    ),
                  ),
                ],
              ),
              ...
              child: Column(
                children: [
                  ...
                  SwitchListTile(
                    ...
                    subtitle: Text(
                      backup.isEnabled
                          ? 'Backup is on. ${backup.frequency.label} backup is selected.'
                          : 'Backup is off. Connect Drive before enabling automatic backups.',
                      style: const TextStyle(
                        color: Color(0xFF7B6A86),
                        fontSize: 11.5,
                        fontWeight: FontWeight.w700,
                  ...
                  Wrap(
                    ...
                    children: ChatBackupFrequency.values.map((item) {
                      ...
                      return InkWell(
                        onTap: backup.isEnabled && !_backupBusy
                            ? () => _setFrequency(item)
                            : null,
              ...
              child: Column(
                children: [
                  _ActionRow(
                    ...
                    title: 'Back up now',
                    subtitle: backup.lastBackupAt == null
                        ? 'Create your first encrypted Drive backup.'
                        : 'Last backup: ${backup.lastBackupAt}',
                    onTap: _backupBusy ? () {} : _backupNow,
                  ),
                  const Divider(color: Color(0xFFECE2D8)),
                  _ActionRow(
                    icon: Icons.restore_rounded,
                    title: 'Restore from backup',
                    subtitle: backup.lastRestoreAt == null
                        ? 'Restore latest available Drive backup.'
                        : 'Last restore: ${backup.lastRestoreAt}',
                    onTap: _backupBusy ? () {} : _restore,
                  ),
                ],
              ),
            ),
            ...
            const _SettingsCard(
              child: Row(
                ...
                children: [
                  ...
                  Expanded(
                    child: Text(
                      'Inbox lock, privacy settings, Secret Drift, backups, and locked-chat recovery are backend-backed. Device unlock is a local shortcut only and never sends fingerprint data to the backend.',
                      style: TextStyle(
                        color: Color(0xFF7B6A86),
                        fontSize: 12,
                        height: 1.35,
...
class _GoogleDriveSetupSheetState extends State<_GoogleDriveSetupSheet> {
  static const String _driveSetupCode = 'dev_mock_drive_code';
  final _email = TextEditingController();
  final _code = TextEditingController();
  bool _busy = false;
  String? _authUrl;
  String? _resolvedSetupCode;
  ...
  Future<void> _start() async {
    ...
    try {
      ...
      final usesInlineCode = url.contains(_driveSetupCode);
      if (usesInlineCode) {
        _resolvedSetupCode = _driveSetupCode;
        _code.text = 'Ready to connect';
      } else {
        _resolvedSetupCode = null;
      }
      setState(() => _authUrl = usesInlineCode ? null : url);
      _toast(
        usesInlineCode
  ...
  Future<void> _connect() async {
    ...
    try {
      ...
      await widget.onConnect(
        ...
        _resolvedSetupCode ??
            (trimmedCode.isEmpty ? _driveSetupCode : trimmedCode),
      );
        ...
        child: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              const Text(
                'Authorize Google Drive so Vibe Match can store encrypted Inbox backups and restore them later.',
                style: TextStyle(
                  color: Color(0xFF7B6A86),
                  fontSize: 12.2,
                  height: 1.35,
  ...
  Widget build(BuildContext context) {
    return InkWell(
      ...
      child: Container(
        height: 48,
        decoration: BoxDecoration(
          color: dark ? const Color(0xFF251538) : const Color(0xFFFAF7F1),
          borderRadius: BorderRadius.circular(18),
          border: Border.all(
            color: dark ? const Color(0xFF251538) : const Color(0xFFECE2D8),
          ),
        ),
        child: Center(
          child: Row(
            ...
            children: [
              Icon(
                icon,
                color: dark ? Colors.white : const Color(0xFF4A2A63),
                size: 18,
              ),
              const SizedBox(width: 6),
              Text(
                label,
                style: TextStyle(
                  color: dark ? Colors.white : const Color(0xFF251538),
                  fontWeight: FontWeight.w900,
                ),
              ),
            ],
