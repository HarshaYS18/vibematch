...
class _InboxLockSetupSheetState extends State<InboxLockSetupSheet> {
  ...
  Future<void> _sendOtp() async {
    ...
    if (mobile.isEmpty) {
      _toast('Enter your recovery mobile number.');
      return;
    }
    setState(() => _busy = true);
    try {
      final otp = await widget.onStartOtp(mobile);
      setState(() {
        _otpSent = true;
        _debugOtp = otp;
      });
      _toast(otp == null ? 'OTP sent for Inbox lock.' : 'OTP sent: $otp');
    } catch (_) {
  ...
  Widget build(BuildContext context) {
    return _LockSheetScaffold(
      ...
      subtitle:
          'Verify your recovery mobile, then create your private chat lock.',
      child: Column(
        children: [
          _LockField(
            controller: _mobile,
          ...
          _PrimaryLockButton(
            label: _otpSent ? 'Confirm setup' : 'Send OTP',
            busy: _busy,
...
class _InboxLockRecoverySheetState extends State<InboxLockRecoverySheet> {
  ...
  void initState() {
    super.initState();
    _mobile.text = widget.registeredMobile?.trim() ?? '';
  }
  ...
  Future<void> _sendOtp() async {
    ...
    setState(() => _busy = true);
    try {
      final otp = await widget.onStartRecovery(mobile);
      setState(() {
        _otpSent = true;
        _debugOtp = otp;
      });
      _toast(otp == null ? 'Recovery OTP sent.' : 'Recovery OTP: $otp');
    } catch (_) {
      _toast('Could not send recovery OTP.');
    } finally {
      if (mounted) setState(() => _busy = false);
    }
  }
  ...
  Widget build(BuildContext context) {
    return _LockSheetScaffold(
      ...
      subtitle:
          'Verify the recovery mobile linked to your Inbox lock, then set a new lock.',
      child: Column(
        children: [
          _LockField(
            controller: _mobile,
          ...
          const SizedBox(height: 14),
          _PrimaryLockButton(
            label: _otpSent ? 'Recover lock' : 'Send recovery OTP',
            busy: _busy,
