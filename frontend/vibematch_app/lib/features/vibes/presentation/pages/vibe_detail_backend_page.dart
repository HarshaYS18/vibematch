  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          if (_isSelfVibe)
            IconButton(onPressed: _deleting ? null : _deleteVibe, icon: Icon(_deleting ? Icons.hourglass_top_rounded : Icons.delete_outline_rounded, color: const Color(0xFFE84C72), size: 24))
          else
              ...
              child: RefreshIndicator(
                ...
                child: CustomScrollView(
                  ...
                  slivers: [
                    ...
                    SliverToBoxAdapter(child: _CommentsHeader(count: _comments.length, loading: _loadingComments, onRefresh: _loadComments)),
...
class _DetailPost extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return ColoredBox(color: Colors.white, child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [_AuthorRow(vibe: vibe), SizedBox(width: size, height: size, child: ClipRect(child: VibeMediaPlayer(vibe: vibe, onDoubleTap: () {}))), _PostInfo(vibe: vibe, caption: _caption), const Divider(height: 1, color: Color(0xFFECE2D8))]));
  }
}
...
class _AuthorRow extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return SizedBox(height: 58, child: Padding(padding: const EdgeInsets.fromLTRB(14, 8, 14, 8), child: Row(children: [VibeAvatar(vibe: vibe, size: 38), const SizedBox(width: 10), Expanded(child: Column(mainAxisAlignment: MainAxisAlignment.center, crossAxisAlignment: CrossAxisAlignment.start, children: [Text(vibe.authorName, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: Color(0xFF111015), fontSize: 14, fontWeight: FontWeight.w900)), const SizedBox(height: 2), Text(vibe.timeAgo, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: Color(0xFF8C8198), fontSize: 11.5, fontWeight: FontWeight.w700))]))])));
  }
}
...
class _PostInfo extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return Padding(padding: const EdgeInsets.fromLTRB(14, 12, 14, 14), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Row(children: [_Metric(icon: Icons.favorite_rounded, value: vibe.likes), const SizedBox(width: 14), _Metric(icon: Icons.mode_comment_rounded, value: vibe.comments), const SizedBox(width: 14), _Metric(icon: Icons.send_rounded, value: vibe.shares), const SizedBox(width: 14), _Metric(icon: Icons.bookmark_rounded, value: vibe.saves)]), const SizedBox(height: 10), if (caption.isNotEmpty) _Caption(authorName: vibe.authorName, caption: caption), const SizedBox(height: 8), Text(vibe.timeAgo.toUpperCase(), style: const TextStyle(color: Color(0xFF8C8198), fontSize: 10.5, fontWeight: FontWeight.w800, letterSpacing: 0.2))]));
  }
}
...
class _Caption extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return RichText(text: TextSpan(style: const TextStyle(color: Color(0xFF111015), fontSize: 13.5, height: 1.35, fontWeight: FontWeight.w600), children: spans));
  }
}
...
class _CommentsHeader extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => Container(color: Colors.white, padding: const EdgeInsets.fromLTRB(14, 14, 8, 8), child: Row(children: [const Expanded(child: Text('Comments', style: TextStyle(color: Color(0xFF111015), fontSize: 17, fontWeight: FontWeight.w900))), if (loading) const SizedBox(width: 16, height: 16, child: CircularProgressIndicator(strokeWidth: 2, color: Color(0xFF111015))) else Text('$count', style: const TextStyle(color: Color(0xFF8C8198), fontSize: 12.5, fontWeight: FontWeight.w900)), IconButton(onPressed: onRefresh, icon: const Icon(Icons.refresh_rounded, color: Color(0xFF111015), size: 20))]));
}
