...
class _VibeDetailBackendPageState extends State<VibeDetailBackendPage> {
  ...
  Future<void> _sendComment() async {
    ...
    if (!_hasBackendId) {
      _toast('This old mock Vibe has no backend ID. Refresh feed and open a backend Vibe.');
      return;
    }
  ...
  void _reportVibe() {
    _toast('Report submitted to CS CP for review.');
  }
        ...
        child: Column(
          children: [
            Container(
              ...
              child: Row(
                children: [
                  _RoundIconButton(icon: VMIcons.back, onTap: () => Navigator.pop(context)),
                  ...
                  _HeaderActionButton(
                    icon: _isSelfVibe ? Icons.delete_rounded : Icons.report_rounded,
                    label: _deleting ? '...' : (_isSelfVibe ? 'Delete' : 'Report'),
                    color: _isSelfVibe ? const Color(0xFFE84C72) : const Color(0xFFC99A3B),
                    onTap: _isSelfVibe ? _deleteVibe : _reportVibe,
                  ),
                ],
              ),
            ),
                  ...
                  children: [
                    ...
                    Row(
                      children: [
                        ...
                        if (_loadingComments)
                          const SizedBox(width: 16, height: 16, child: CircularProgressIndicator(strokeWidth: 2, color: Color(0xFF251538)))
                        else
                          Text('${_comments.length} total', style: const TextStyle(color: Color(0xFF7B6A86), fontSize: 12, fontWeight: FontWeight.w800)),
                        IconButton(onPressed: _loadComments, icon: const Icon(Icons.refresh_rounded, color: Color(0xFF4A2A63), size: 19)),
                      ],
                    ),
                    if (_error != null) ...[
                    ...
                    if (_comments.isEmpty && !_loadingComments)
                      ...
                      ..._comments.map((comment) => Padding(padding: const EdgeInsets.only(bottom: 12), child: _CommentCard(comment: comment))),
                  ],
                ),
              ),
            ),
            _CommentComposer(controller: _commentController, sending: _sendingComment, onSend: _sendComment),
          ],
        ),
      ),
    );
...
class _ContentCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
        minHeight: vibe.mediaType == VibeMediaType.text ? 150 : 300,
        padding: const EdgeInsets.all(22),
        decoration: BoxDecoration(borderRadius: BorderRadius.circular(28), gradient: LinearGradient(begin: Alignment.topLeft, end: Alignment.bottomRight, colors: vibe.colors), boxShadow: [BoxShadow(color: vibe.colors.first.withValues(alpha: 0.20), blurRadius: 18, offset: const Offset(0, 9))]),
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Icon(vibe.mediaType.icon, color: Colors.white.withValues(alpha: 0.86), size: 48),
          const SizedBox(height: 18),
          Text(vibe.caption, style: const TextStyle(color: Colors.white, fontSize: 20, height: 1.28, fontWeight: FontWeight.w900)),
        ]),
      );
}
...
class _CommentCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Expanded(child: _WhiteCard(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
...
class _CommentComposer extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
        ...
        child: Row(children: [
          Expanded(child: TextField(controller: controller, enabled: !sending, decoration: InputDecoration(hintText: 'Comment with @name or @all...', hintStyle: const TextStyle(color: Color(0xFF8C8198), fontWeight: FontWeight.w700), filled: true, fillColor: const Color(0xFFFAF7F1), contentPadding: const EdgeInsets.symmetric(horizontal: 14, vertical: 12), border: OutlineInputBorder(borderRadius: BorderRadius.circular(18), borderSide: const BorderSide(color: Color(0xFFECE2D8))), enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(18), borderSide: const BorderSide(color: Color(0xFFECE2D8))), focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(18), borderSide: const BorderSide(color: Color(0xFF12C7B7))))),),
          const SizedBox(width: 10),
          InkWell(onTap: sending ? null : onSend, borderRadius: BorderRadius.circular(18), child: Container(height: 48, width: 48, decoration: BoxDecoration(color: const Color(0xFF251538), borderRadius: BorderRadius.circular(18)), child: sending ? const Padding(padding: EdgeInsets.all(13), child: CircularProgressIndicator(strokeWidth: 2, color: Colors.white)) : const Icon(VMIcons.send, color: Colors.white))),
        ]),
      );
}
...
class _EmptyCommentsCard extends StatelessWidget {
  const _EmptyCommentsCard();
  @override
  Widget build(BuildContext context) => _WhiteCard(child: const Text('No comments yet. Be the first to comment.', style: TextStyle(color: Color(0xFF7B6A86), fontWeight: FontWeight.w800)));
}
...
class _ConfirmSheet extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => Container(margin: const EdgeInsets.all(14), padding: EdgeInsets.fromLTRB(18, 16, 18, 18 + MediaQuery.paddingOf(context).bottom), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(30)), child: Column(mainAxisSize: MainAxisSize.min, children: [Text(title, style: const TextStyle(color: Color(0xFF251538), fontSize: 21, fontWeight: FontWeight.w900)), const SizedBox(height: 8), Text(body, textAlign: TextAlign.center, style: const TextStyle(color: Color(0xFF7B6A86), fontWeight: FontWeight.w700)), const SizedBox(height: 16), Row(children: [Expanded(child: OutlinedButton(onPressed: () => Navigator.pop(context, false), child: const Text('Cancel'))), const SizedBox(width: 10), Expanded(child: ElevatedButton(style: ElevatedButton.styleFrom(backgroundColor: confirmColor, foregroundColor: Colors.white), onPressed: () => Navigator.pop(context, true), child: Text(confirmText)))])]));
}
