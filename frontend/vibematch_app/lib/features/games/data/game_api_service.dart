...
class GameApiService {
  ...
  Future<List<GameDefinition>> loadCatalog() async {
    final response = await http.get(Uri.parse(VmApiConfig.endpoint('/games/catalog')));
    _throwIfFailed(response, 'load game catalog');
    final decoded = jsonDecode(response.body) as Map<String, dynamic>;
    final games = decoded['games'] as List<dynamic>? ?? const [];
    return games.whereType<Map<String, dynamic>>().map(GameDefinition.fromJson).toList(growable: false);
  }

  Future<GameDefinition> seedDefaultGames() async {
    final response = await http.post(Uri.parse(VmApiConfig.endpoint('/games/admin/seed-defaults')), headers: await _headers());
    _throwIfFailed(response, 'seed default games');
    return GameDefinition.fromJson(jsonDecode(response.body) as Map<String, dynamic>);
  }
  ...
  Future<GameRound> createRound({required String gameKey, int? roomId}) async {
    ...
    return GameRound.fromJson(jsonDecode(response.body) as Map<String, dynamic>);
  }

  Future<GameRound> getRound(int roundId) async {
    final response = await http.get(Uri.parse(VmApiConfig.endpoint('/games/rounds/$roundId')), headers: await _headers());
    _throwIfFailed(response, 'load game round');
    return GameRound.fromJson(jsonDecode(response.body) as Map<String, dynamic>);
  }

  Future<GameBetResult> placeBet({required int roundId, required int targetId, required int amount}) async {
    final response = await http.post(
    ...
    return GameBetResult.fromJson(jsonDecode(response.body) as Map<String, dynamic>);
  }

  Future<GameRoundResult> settleTestRound(int roundId) async {
    final response = await http.post(Uri.parse(VmApiConfig.endpoint('/games/rounds/$roundId/settle-test')), headers: await _headers());
    _throwIfFailed(response, 'settle game round');
    return GameRoundResult.fromJson(jsonDecode(response.body) as Map<String, dynamic>);
  }
  ...
  void _throwIfFailed(http.Response response, String action) {
    ...
    throw Exception('Game API failed to $action (${response.statusCode}): ${response.body}');
  }
}
...
class GameDefinition {
  ...
  List<GameTarget> get targets {
    ...
    return rawTargets.whereType<Map<String, dynamic>>().map(GameTarget.fromJson).toList(growable: false);
  }
...
class GameTarget {
  const GameTarget({required this.id, required this.label, required this.emoji, required this.multiplier, required this.themeColor});
...
class GameRound {
  const GameRound({required this.id, required this.gameKey, required this.roomId, required this.status, required this.entryFee, required this.roundPoolAmount, required this.metadata});
...
class GameBetResult {
  const GameBetResult({required this.betId, required this.roundId, required this.targetId, required this.requestedAmount, required this.acceptedAmount, required this.walletCoinBalance, required this.riskLevel, required this.riskScore, required this.riskAction, required this.message});
  ...
  bool get shouldShowLoadingOverlay {
    ...
    return acceptedAmount <= 0 || normalized == 'TAP_IGNORED_COOLDOWN' || normalized == 'IGNORE' || normalized == 'IGNORED';
  }
