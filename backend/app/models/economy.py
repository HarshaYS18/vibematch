...
class WalletLedger(Base):
    ...
    currency_type: Mapped[EconomyCurrency] = mapped_column(String(20), index=True, nullable=False)
    direction: Mapped[EconomyDirection] = mapped_column(String(20), index=True, nullable=False)
...
class CoinSupplyPool(Base):
    ...
    pool_type: Mapped[CoinSupplyPoolType] = mapped_column(String(50), index=True, nullable=False)
    balance: Mapped[int] = mapped_column(BigInteger, default=0, nullable=False)
    reserved_balance: Mapped[int] = mapped_column(BigInteger, default=0, nullable=False)
    status: Mapped[EconomyPoolStatus] = mapped_column(String(20), default=EconomyPoolStatus.ACTIVE.value, index=True)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow)
    updated_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
...
class GamePool(Base):
    ...
    pool_type: Mapped[GamePoolType] = mapped_column(String(50), index=True, nullable=False)
    balance: Mapped[int] = mapped_column(BigInteger, default=0, nullable=False)
    reserved_balance: Mapped[int] = mapped_column(BigInteger, default=0, nullable=False)
    status: Mapped[EconomyPoolStatus] = mapped_column(String(20), default=EconomyPoolStatus.ACTIVE.value, index=True)
...
class CoinSaleOrder(Base):
    ...
    delivery_status: Mapped[CoinSaleStatus] = mapped_column(String(30), default=CoinSaleStatus.PENDING.value, index=True)
...
class RubyWithdrawRequest(Base):
    ...
    status: Mapped[RubyWithdrawStatus] = mapped_column(String(30), default=RubyWithdrawStatus.PENDING.value, index=True)
...
class GameRound(Base):
    ...
    game_key: Mapped[str] = mapped_column(String(80), index=True, nullable=False)
    room_id: Mapped[int | None] = mapped_column(ForeignKey("rooms.id"), index=True, nullable=True)
    status: Mapped[GameRoundStatus] = mapped_column(String(30), default=GameRoundStatus.WAITING.value, index=True)
    entry_fee: Mapped[int] = mapped_column(BigInteger, default=0, nullable=False)
    currency_type: Mapped[EconomyCurrency] = mapped_column(String(20), default=EconomyCurrency.COIN.value, nullable=False)
