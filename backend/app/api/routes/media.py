...
def _save_upload(file: UploadFile, *, folder: str, max_size: int, allowed_types: set[str], request: Request) -> tuple[MediaUploadResponse, str]:
...
def upload_room_avatar(request: Request, file: UploadFile = File(...), current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    result, object_key = _save_upload(file, folder=f"room_avatars/user_{current_user.id}", max_size=MAX_ROOM_AVATAR_BYTES, allowed_types=ALLOWED_IMAGE_TYPES, request=request)
...
def upload_room_cover(request: Request, file: UploadFile = File(...), current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    result, object_key = _save_upload(file, folder=f"room_covers/user_{current_user.id}", max_size=MAX_ROOM_COVER_BYTES, allowed_types=ALLOWED_IMAGE_TYPES, request=request)
...
def upload_room_background(request: Request, file: UploadFile = File(...), current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    result, object_key = _save_upload(file, folder=f"room_backgrounds/user_{current_user.id}", max_size=MAX_ROOM_BACKGROUND_BYTES, allowed_types=ALLOWED_IMAGE_TYPES, request=request)
...
def upload_home_banner(request: Request, file: UploadFile = File(...), current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    result, object_key = _save_upload(file, folder=f"home_banners/user_{current_user.id}", max_size=MAX_HOME_BANNER_BYTES, allowed_types=ALLOWED_IMAGE_TYPES, request=request)
...
def upload_chat_image(request: Request, file: UploadFile = File(...), current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    result, object_key = _save_upload(file, folder=f"chat_images/user_{current_user.id}", max_size=MAX_CHAT_IMAGE_BYTES, allowed_types=ALLOWED_IMAGE_TYPES, request=request)
...
def upload_chat_document(request: Request, file: UploadFile = File(...), current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    result, object_key = _save_upload(file, folder=f"chat_documents/user_{current_user.id}", max_size=MAX_CHAT_DOCUMENT_BYTES, allowed_types=ALLOWED_DOCUMENT_TYPES, request=request)
    asset = cdn_media_service.register_uploaded_media(db, owner=current_user, media_type=CdnMediaType.INBOX_MEDIA, public_url=result.url, object_key=object_key, mime_type=result.content_type, size_bytes=result.size_bytes, linked_entity_type=CdnMediaLinkedEntityType.INBOX_MESSAGE)
    return _with_asset(result, asset)


@router.post("/chat-voice", response_model=MediaUploadResponse)
def upload_chat_voice(request: Request, file: UploadFile = File(...), current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    result, object_key = _save_upload(file, folder=f"chat_voice/user_{current_user.id}", max_size=MAX_CHAT_VOICE_BYTES, allowed_types=ALLOWED_AUDIO_TYPES, request=request)
    asset = cdn_media_service.register_uploaded_media(db, owner=current_user, media_type=CdnMediaType.INBOX_MEDIA, public_url=result.url, object_key=object_key, mime_type=result.content_type, size_bytes=result.size_bytes, linked_entity_type=CdnMediaLinkedEntityType.INBOX_MESSAGE)
    return _with_asset(result, asset)
