...
def _fail_if_banned_or_inactive(db: Session, user: User, email: str, provider: str, provider_user_id: str, device_id: str | None, client_ip: str | None) -> None:
    if user.is_banned:
        create_login_security_log(db=db, action=AuditAction.LOGIN_FAILED_USER_BANNED, email=email, device_id=device_id, target_user_id=user.id, reason="Login blocked because user account is banned.", ip_address=client_ip, extra_metadata={"provider": provider, "provider_user_id": provider_user_id, "public_user_id": user.public_user_id})
        create_login_history(db=db, user_id=user.id, email=email, provider=provider, provider_user_id=provider_user_id, status=LoginHistoryStatus.FAILED, is_success=False, device_id=device_id, ip_address=client_ip, failure_reason=LoginHistoryFailureReason.USER_BANNED, failure_detail="User account is banned.")
        raise HTTPException(status_code=403, detail="User is banned")
    if not user.is_active:
        create_login_security_log(db=db, action=AuditAction.LOGIN_FAILED_USER_INACTIVE, email=email, device_id=device_id, target_user_id=user.id, reason="Login blocked because user account is inactive.", ip_address=client_ip, extra_metadata={"provider": provider, "provider_user_id": provider_user_id, "public_user_id": user.public_user_id})
        create_login_history(db=db, user_id=user.id, email=email, provider=provider, provider_user_id=provider_user_id, status=LoginHistoryStatus.FAILED, is_success=False, device_id=device_id, ip_address=client_ip, failure_reason=LoginHistoryFailureReason.USER_INACTIVE, failure_detail="User account is inactive.")
...
def _fail_if_device_banned(db: Session, email: str, provider: str, provider_user_id: str, device_id: str | None, client_ip: str | None) -> None:
    if is_device_banned(db, device_id):
        create_login_security_log(db=db, action=AuditAction.LOGIN_FAILED_DEVICE_BANNED, email=email, device_id=device_id, reason="Login blocked because device_id has an active device ban.", ip_address=client_ip, extra_metadata={"provider": provider, "provider_user_id": provider_user_id})
        create_login_history(db=db, email=email, provider=provider, provider_user_id=provider_user_id, status=LoginHistoryStatus.FAILED, is_success=False, device_id=device_id, ip_address=client_ip, failure_reason=LoginHistoryFailureReason.DEVICE_BANNED, failure_detail="This device has been permanently banned.")
...
def _get_or_create_identity_user(db: Session, provider: str, provider_user_id: str, email: str, username: str | None, display_name: str | None, avatar_url: str | None = None) -> User:
    identity = db.query(AuthIdentity).filter(AuthIdentity.provider == provider, AuthIdentity.provider_user_id == provider_user_id).first()
    if identity:
        user = identity.user
        changed = False
...
def _google_profile_from_payload(payload: GoogleLoginRequest) -> dict:
    ...
    if access_token:
        return _google_profile_from_access_token(access_token)
    raise HTTPException(status_code=401, detail="Google login token missing.")
