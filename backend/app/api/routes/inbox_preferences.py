...
from app.schemas.inbox import (
    ...
    InboxConversationThemeRequest,
    InboxPreferenceResponse,
    InboxPreferenceUpdateRequest,
)
from app.services import inbox_preference_service, inbox_service
from app.websocket.inbox_ws import inbox_ws_manager

router = APIRouter(prefix="/inbox", tags=["Inbox Preferences"])
...
def get_preferences(db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    preference = inbox_preference_service.get_or_create_preferences(db, current_user)
    return InboxPreferenceResponse(**inbox_preference_service.serialize_preferences(preference))
...
def update_preferences(request: InboxPreferenceUpdateRequest, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    try:
        preference = inbox_preference_service.update_preferences(
            db=db,
            user=current_user,
