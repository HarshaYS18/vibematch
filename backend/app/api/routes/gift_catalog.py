...
def seed_default_gift_catalog(
    ...
    _audit(
        db,
        actor=current_user,
        action="GIFT_CATALOG_SEED_DEFAULTS",
        resource_id="catalog",
...
def upsert_gift_category(
    category_key: str,
    payload: GiftCategoryAdminPayload,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    _require_gift_catalog_admin(current_user)
...
def set_gift_category_enabled(
    ...
    _audit(
        db,
        actor=current_user,
        action="GIFT_CATEGORY_ENABLED_CHANGED",
        resource_id=key,
        reason=payload.reason,
        metadata={"is_enabled": payload.is_enabled},
    )
    db.commit()
    db.refresh(category)
    return _category_response(category)
...
def upsert_gift_item(
    gift_id: str,
    payload: GiftItemAdminPayload,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    _require_gift_catalog_admin(current_user)
...
def set_gift_item_enabled(
    gift_id: str,
    payload: GiftItemEnabledPayload,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    _require_gift_catalog_admin(current_user)
    ...
    _audit(
        db,
        actor=current_user,
        action="GIFT_ITEM_ENABLED_CHANGED",
        resource_id=gift_id.strip(),
        reason=payload.reason,
        metadata={"is_enabled": payload.is_enabled},
    )
    db.commit()
