...
async def _broadcast_gift_experience_updates(*, db: Session, current_user_id: int, receiver_user_id: int, result: dict) -> None:
    exp_updates = result.get("experience_updates") if isinstance(result.get("experience_updates"), dict) else {}
    await inbox_ws_manager.send_to_user(current_user_id, {"event": "experience_updated", "scope": "send", "payload": exp_updates.get("sender")})
    ...
    if exp_updates.get("room") is not None:
        await inbox_ws_manager.broadcast_to_users([current_user_id, receiver_user_id], {"event": "room_experience_updated", "payload": exp_updates.get("room")})
...
def _validate_catalog_gift(db: Session, gift_id: str, coin_value: int, quantity: int) -> dict:
...
async def send_gift_by_public_ids(payload: PublicGiftSendRequest, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    _validate_catalog_gift(db, payload.gift_id, payload.coin_value, payload.quantity)
    receiver, room_id = _resolve_receiver_and_room(db, payload)
    result = economy_service.send_gift(db=db, sender=current_user, receiver_user_id=receiver.id, gift_id=payload.gift_id, coin_value=payload.coin_value, quantity=payload.quantity, room_id=room_id, relationship_id=payload.relationship_id, is_relationship_gift=payload.is_relationship_gift)
    await _broadcast_gift_experience_updates(db=db, current_user_id=current_user.id, receiver_user_id=receiver.id, result=result)
    return result


@router.post("/send-lucky-public")
async def send_lucky_gift_by_public_ids(payload: PublicLuckyGiftSendRequest, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    gift = _validate_catalog_gift(db, payload.gift_id, payload.coin_value, payload.quantity)
    ...
    try:
        ...
        result = economy_service.send_gift(db=db, sender=current_user, receiver_user_id=receiver.id, gift_id=payload.gift_id, coin_value=payload.coin_value, quantity=payload.quantity, room_id=room_id, relationship_id=payload.relationship_id, is_relationship_gift=payload.is_relationship_gift, commit=False)
        ...
        lucky_result = lucky_gift_props_service.roll_lucky_gift(db, gift_id=payload.gift_id, gift_name=str(gift.get("name") or payload.gift_id.replace("_", " ").title()), base_coin_value=payload.coin_value, quantity=payload.quantity, house_risk_score=payload.house_risk_score)
        ...
        reward_wallet = economy_service.credit_lucky_gift_reward(db=db, user_id=current_user.id, reward_coin_amount=reward_coins, source_id=f"lucky_gift:{result['gift_transaction_id']}", created_by_user_id=current_user.id, commit=False)
        lucky_tx, _ = lucky_gift_stats_service.record_lucky_gift_result(db, sender_user_id=current_user.id, receiver_user_id=receiver.id, room_id=room_id, gift_id=payload.gift_id, gift_name=str(gift.get("name") or payload.gift_id.replace("_", " ").title()), coin_value=payload.coin_value, quantity=payload.quantity, spent_coins=spent_coins, multiplier=multiplier, reward_coins=reward_coins, net_win_coins=reward_coins - spent_coins, metadata_json=json.dumps({"gift_transaction_id": result["gift_transaction_id"], "source": "send_lucky_public", "lucky_result": lucky_result, "risk": risk_result, "house": house_result}, separators=(",", ":")))
        db.commit()
    ...
    await _broadcast_gift_experience_updates(db=db, current_user_id=current_user.id, receiver_user_id=receiver.id, result=result)
    await inbox_ws_manager.send_to_user(current_user.id, {"event": "lucky_gift_result", "payload": {"gift_transaction_id": result["gift_transaction_id"], "gift_id": payload.gift_id, "receiver_user_id": receiver.id, "room_public_id": payload.room_public_id, "lucky_result": lucky_result, "sender_coin_balance": reward_wallet.coin_balance}})
    return result