...
# The visible level number must always come from these tables through progress_payload().
INR_TO_COIN_EXP_RATE = 1_000
VIP_MAX_LEVEL = 50
SVIP_MAX_LEVEL = 10
STANDARD_MAX_LEVEL = 100

# VIP progression is intentionally smoother than the older table:
# - early levels unlock quickly so normal users can see progression;
# - mid levels climb predictably;
# - upper levels still represent true whale/elite lifetime recharge status;
# - VIP 50 remains the long-term 50B coin EXP cap.
...
# SVIP is monthly recharge status. It should feel premium, but not use the same
# level shape as lifetime VIP. SVIP 10 stays the 2B monthly recharge cap.
