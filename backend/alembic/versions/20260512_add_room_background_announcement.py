...
def upgrade() -> None:
    ...
    op.add_column("rooms", sa.Column("announcement_text", sa.Text(), nullable=True))
    op.add_column("rooms", sa.Column("announcement_updated_at", sa.DateTime(), nullable=True))
    op.add_column("rooms", sa.Column("announcement_updated_by_user_id", sa.Integer(), nullable=True))
    op.create_index("ix_rooms_background_theme_id", "rooms", ["background_theme_id"])
