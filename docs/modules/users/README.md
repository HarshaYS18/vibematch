...
Maintains canonical user identity and assembles master user snapshots from child domains.

## Responsibilities

The module owns users; master-state read model from child tables. Routes should validate input and delegate business decisions to services.

## What this module owns

users; master-state read model from child tables.
...
PostgreSQL is the durable source of truth for users and related child tables read by snapshot.

## Important files

`backend/app/api/routes/users.py`, `backend/app/services/identity_service.py`, `backend/app/services/user_master_state_service.py`.

## Public API/contracts

/api/v1/users routes and master-state snapshot. Preserve deployed request and response shapes while migrating implementation.
...
Database tables and state: `users and related child tables read by snapshot`.

## Redis keys/state owned

No durable identity fields in Redis.
...
Public identifiers and privacy filtering require server-side validation. Never log tokens, OTPs, or payment secrets.

## Failure modes

A partial child-domain outage should not silently fabricate authoritative values.
