...
Collects traces, metrics, and structured logs across requests, events, workers, and media. Telemetry contracts and dashboards; never domain state.
...
`backend/app/main.py`, `backend/app/api/routes/health.py`, `backend_media/src/server.ts`, `deploy (target)`. Contract surface: /health and media /health /ready currently; /live /ready /metrics as rollout contracts. Keep existing Flutter-compatible paths and payloads until a versioned migration is ready.
...
Database: No business tables; retention belongs to observability platform. Redis/Valkey: No application authority in Redis.

## Dependencies and security

Dependencies include the configured runtime, PostgreSQL, Redis/Valkey, and relevant internal contracts where applicable. Redact JWTs, OTPs, passwords, payment data, private message bodies, and secret keys. Do not log tokens, credentials, private content, or payment secrets.

## Failure modes and retry/idempotency

Loss of telemetry must not silently mark unhealthy business writes as successful. Use bounded timeouts and explicit retry budgets. Only replay writes when a stable idempotency key or reconciliation proves the commit outcome.
