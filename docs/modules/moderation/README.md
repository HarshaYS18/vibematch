...
Records cases, evidence, bans, actions, and review decisions. The module owns moderation_cases, moderation_evidence, moderation_events, user_bans, device_bans. Routes should validate input and delegate business decisions to services.

## Ownership and source of truth

PostgreSQL is the durable source of truth for moderation_cases, moderation_evidence, moderation_events, user_bans, device_bans. This module does not own SFU transport state, edge routing, or client UI state. Flutter caches are replaced by backend snapshots.

## Important files and public contracts

`backend/app/services/moderation_case_service.py`, `backend/app/services/ban_service.py`, `backend/app/api/routes/moderation.py`. Public surface: moderation, bans, and media safety admin routes. Preserve deployed request and response shapes while migrating implementation.
...
Database tables and state: `moderation_cases, moderation_evidence, moderation_events, user_bans, device_bans`. Redis: Rate-limit counters may be transient; bans cannot be Redis-only.

## Dependencies and security

Depends on FastAPI authentication, SQLAlchemy transaction/session handling, and the relevant domain services. Privileged actions require roles, audit trails, and evidence access control. Never log tokens, OTPs, or payment secrets.

## Failure modes and retry behavior

If ban lookup fails, sensitive participation must not be granted based on stale cache. Reads and writes should use bounded timeouts. Retries are safe only for read operations or writes backed by an idempotency key and known commit outcome.
