...
Keeps one ordered schema history and verifies application/schema compatibility. Alembic revisions under backend/alembic/versions.
...
`backend/alembic/env.py`, `backend/alembic/versions`, `backend/app/core/schema_guard.py`. Contract surface: alembic upgrade head; alembic heads. Keep existing Flutter-compatible paths and payloads until a versioned migration is ready.
...
Database: All production schema changes; no separate Go graph. Redis/Valkey: None.

## Dependencies and security

Dependencies include the configured runtime, PostgreSQL, Redis/Valkey, and relevant internal contracts where applicable. Run with dedicated credentials and avoid exposing data in logs. Do not log tokens, credentials, private content, or payment secrets.

## Failure modes and retry/idempotency

Failed migration halts deployment; restore only from tested backup with a plan. Use bounded timeouts and explicit retry budgets. Only replay writes when a stable idempotency key or reconciliation proves the commit outcome.
