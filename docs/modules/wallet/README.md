...
Owns balances and the ledger for all value movement. The module owns user_wallets and wallet_ledger. Routes should validate input and delegate business decisions to services.

## Ownership and source of truth

PostgreSQL is the durable source of truth for user_wallets, wallet_ledger. This module does not own SFU transport state, edge routing, or client UI state. Flutter caches are replaced by backend snapshots.

## Important files and public contracts

`backend/app/models/economy.py`, `backend/app/api/routes/wallet`, `backend/app/services/economy_service.py`. Public surface: wallet routes under /api/v1. Preserve deployed request and response shapes while migrating implementation.
...
Database tables and state: `user_wallets, wallet_ledger`. Redis: No wallet balance or ledger authority in Redis.

## Dependencies and security

Depends on FastAPI authentication, SQLAlchemy transaction/session handling, and the relevant domain services. Debit and credit must commit atomically with permissions, limits, and idempotency. Never log tokens, OTPs, or payment secrets.

## Failure modes and retry behavior

Never retry a non-idempotent value write without its original idempotency key. Reads and writes should use bounded timeouts. Retries are safe only for read operations or writes backed by an idempotency key and known commit outcome.
