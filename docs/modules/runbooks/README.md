...
Provides repeatable incident and maintenance procedures. Operational response guidance and evidence collection, not runtime control.
...
`docs/runbooks/README.md`, `docs/runbooks`. Contract surface: Human operator procedures. Keep existing Flutter-compatible paths and payloads until a versioned migration is ready.
...
Database: None. Redis/Valkey: None.

## Dependencies and security

Dependencies include the configured runtime, PostgreSQL, Redis/Valkey, and relevant internal contracts where applicable. Limit production commands to approved roles; capture audit trail and avoid secret dumps. Do not log tokens, credentials, private content, or payment secrets.

## Failure modes and retry/idempotency

Use symptoms, immediate actions, safe mitigation, validation, and escalation for each incident. Use bounded timeouts and explicit retry budgets. Only replay writes when a stable idempotency key or reconciliation proves the commit outcome.
