...
IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname='funkey_profile_social_reader') THEN CREATE ROLE funkey_profile_social_reader NOLOGIN; END IF;
END $$;
GRANT USAGE ON SCHEMA public TO funkey_profile_social_runtime;
GRANT USAGE ON SCHEMA public TO funkey_profile_social_reader;
...
GRANT SELECT ON TABLE user_follows,user_blocks,love_bonds,love_bond_requests,profile_visits,user_stealth_states,profile_display_audits,family_economy_stats,family_member_stats TO funkey_profile_social_reader;

-- Profile display is a projection over Economy-owned truth. These grants are
-- deliberately SELECT-only; Profile/Social cannot create wallets, sync VIP,
-- or mutate Economy ledgers/rules.
GRANT SELECT ON TABLE user_wallets,wallet_ledger,gift_transactions,economy_rule_sets,economy_rule_levels TO funkey_profile_social_runtime;
...
-- Deployment binding:
--   GRANT funkey_profile_social_runtime TO <production_profile_social_login>;
--   GRANT funkey_profile_social_reader TO <production_core_api_login>;
-- Core/composite readers receive no social/profile/family mutation privileges.
